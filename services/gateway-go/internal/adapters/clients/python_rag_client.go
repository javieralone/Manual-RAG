...
func NewPythonRAGClient(baseURL string, httpClient *http.Client, metrics *observability.Metrics, values ...ResilienceConfig) *PythonRAGClient {
...
func (c *PythonRAGClient) retrieveContext(ctx context.Context, query string, topK int, filters domain.QueryFilters) ([]domain.DocumentChunk, error) {
	...
	for attempt := 0; attempt <= c.retries; attempt++ {
		req, requestErr := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+"/search", bytes.NewReader(reqBody))
		if requestErr != nil {
			return nil, fmt.Errorf("error creando request HTTP a rag-engine: %w", requestErr)
		}

...
func NewMetrics(registerer prometheus.Registerer) *Metrics {
	m := &Metrics{
		...
		RateLimitRejections: prometheus.NewCounterVec(prometheus.CounterOpts{Name: "api_go_rate_limit_rejections_total", Help: "HTTP requests rejected by rate limiting."}, []string{"scope", "route"}),
		Readiness:           prometheus.NewGauge(prometheus.GaugeOpts{Name: "api_go_readiness", Help: "Gateway readiness: 1 ready, 0 not ready."}),
	}
	registerer.MustRegister(m.RequestsTotal, m.RequestDuration, m.AuthTotal, m.DependencyTotal, m.DependencyDuration, m.GenerationDuration, m.GeneratedTokens, m.TimeToFirstToken, m.ErrorsTotal, m.WorkerInFlight, m.WorkerRejections, m.RateLimitRejections, m.Readiness)
	m.Readiness.Set(0)
	return m
}

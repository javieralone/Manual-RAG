...
from manual_rag.bootstrap import get_rag_service
from manual_rag.domain.schemas import DEFAULT_COLLECTION, ChunkResult, resolve_collection
...
def _load_domain_collections() -> Dict[str, str]:
    ...
    if not raw_mapping:
        return {"generic_manuals": "generic_manuals", "technical_manuals": "manuales_tecnicos"}
    try:
...
def _search(
    ...
    filters = {
        key: value
        for key, value in {"document_id": document_id, "chapter": chapter, "section": section}.items()
